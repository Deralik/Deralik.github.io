...
(()=>{const{rng,fit,loop,tok,cmap,star}=GRT;const{Cam2}=GRT2;const{box}=GRT6;const{CField}=GRT7;const{KC,EYE,vol}=GRT8;
class T8{
...
this.c0=new Cam2(null,.92,.30,4.6);
...
const walk=p=>{for(let a=0;a<40;a++){const th=r()*6.283,ph=Math.acos(2*r()-1),L=.28+.34*r(),q=[p[0]+Math.sin(ph)*Math.cos(th)*L,p[1]+Math.cos(ph)*L,p[2]+Math.sin(ph)*Math.sin(th)*L];if(V.sig(q[0],q[1],q[2])>.06)return q}return p};
const mk=n=>{const a=[EYE.slice()];let p=pick();a.push(p);for(let i=1;i<n;i++){p=walk(p);a.push(p)}return a};
return[mk(1),mk(2),mk(4)]}
mkRays3(){const r=rng(47),V=this.vol,out=[];
const walk=p=>{for(let a=0;a<40;a++){const th=r()*6.283,ph=Math.acos(2*r()-1),L=.26+.32*r(),q=[p[0]+Math.sin(ph)*Math.cos(th)*L,p[1]+Math.cos(ph)*L,p[2]+Math.sin(ph)*Math.sin(th)*L];if(V.sig(q[0],q[1],q[2])>.06)return q}return p};
for(let i=0;i<4;i++){const th=r()*6.283,o=[2.2*Math.cos(th),.4+.5*r(),2.2*Math.sin(th)],V0=[o];
let p=null;for(let k=0;k<220;k++){const tt=.5+k*.03;const q=[o[0]+(0-o[0])*tt/2.4,o[1]+(.05-o[1])*tt/2.4,o[2]+(0-o[2])*tt/2.4];if(Math.abs(q[0])<1&&Math.abs(q[1])<1&&Math.abs(q[2])<1&&V.sig(q[0],q[1],q[2])>.08&&r()<.4){p=q;break}}
if(!p)continue;V0.push(p);const nb=1+(r()*2.4|0);for(let k2=0;k2<nb;k2++){p=walk(p);V0.push(p)}
out.push({V:V0,t0:1.2+i*2.3})}
return out}
mkDepth(){const r=rng(29),V=this.vol,pts=V.samples(250),out=[[],[],[]];
...
for(const r of this.rays){const u=Math.min(1,(t-r.born)/.7);
g.strokeStyle=this.conw;g.globalAlpha=Math.min(1,(1.9-(t-r.born))/.4);g.lineWidth=1;
g.beginPath();g.moveTo(x0+12,r.y);const ex=x0+12+(bx-x0-12)*u,ey=r.y+(h/2-r.y)*u*.9;g.lineTo(ex,ey);g.stroke();
...
const LX=k=>x0+20+sk+k*(lw+gp),LY=h*.50;
...
for(let k=0;k<3;k++){const P=this.paths[k],active=k===act,drawU=active?Math.min(1,ph/.5):1;
g.globalAlpha=active?1:.22;g.strokeStyle=this.conw;g.lineWidth=active?1.3:1;
const nseg=P.length-1;g.beginPath();g.moveTo(cam0[0],cam0[1]);
for(let i=1;i<P.length;i++){const su=Math.min(1,Math.max(0,drawU*nseg-(i-1)));if(su<=0)break;
if(i===1){const pb=px(P[1]);g.lineTo(cam0[0]+(pb[0]-cam0[0])*su,cam0[1]+(pb[1]-cam0[1])*su)}
...
const T=9.2;
for(const R of this.rays3){const lt=(t-R.t0)%T,V=R.V;if(lt<0)continue;
const segs=V.length-1,dur=.55,total=segs*dur+1.6;if(lt>total)continue;
g.globalAlpha=Math.min(1,(total-lt)/.6);
g.strokeStyle=this.conw;g.lineWidth=1.15;g.beginPath();let st=false;
for(let i=1;i<V.length;i++){const s0=(i-1)*dur,u=Math.min(1,Math.max(0,(lt-s0)/dur));if(u<=0)break;
const a=V[i-1],b=V[i],q=[a[0]+(b[0]-a[0])*u,a[1]+(b[1]-a[1])*u,a[2]+(b[2]-a[2])*u];const pa=px(a),pb=px(q);if(!st){g.moveTo(pa[0],pa[1]);st=true}g.lineTo(pb[0],pb[1])}g.stroke();
for(let i=1;i<V.length-1;i++){if(lt<i*dur)break;const q=px(V[i]);g.fillStyle=this.conw;g.beginPath();g.arc(q[0],q[1],1.8,0,6.283);g.fill()}
if(lt>=segs*dur){const q=px(V[V.length-1]),gl=Math.min(1,(lt-segs*dur)/.4);
g.fillStyle=this.cacc;g.globalAlpha=Math.min(1,(total-lt)/.6)*gl;g.fillRect(q[0]-2.5,q[1]-2.5,5,5);
