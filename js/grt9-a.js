...
(() => {
  ...
  function setup(cv, AW, AH, drawBody) {
  ...
  window.GRT9A = function (cv) {
    setup(cv, 640, 262, (ctx, hp, H, mono) => {
      const { T, box, seg, arr, circ, abs, mat } = hp;
      /* washes */
      ctx.fillStyle = tok('--band1');
      ctx.fillRect(16, 64, 150, 150);
      ctx.fillStyle = tok('--hair');
      ctx.globalAlpha = 0.8;
      ctx.fillRect(180, 64, 300, 150);
      ctx.globalAlpha = 1;
      ctx.fillStyle = mat;
      ctx.globalAlpha = 0.45;
      ctx.fillRect(494, 64, 130, 150);
      ctx.globalAlpha = 1;
      ctx.font = '500 9.5px ' + mono;
      T(22, 78, 'THE LOOP', abs, 'left');
      T(186, 78, 'TRAINING — EVERY STEP', abs, 'left');
      T(500, 78, 'WORLD SPACE', abs, 'left');
      ctx.font = '400 9.5px ' + mono;
      /* PCD pass — once, dashed, into the cache */
      box(16, 14, 204, 24, 'PCD pass — first-hit positions');
      seg(220, 26, 559, 26, true);
      arr(559, 26, 559, 98, true);
      circ(388, 26, '1');
      /* the row */
      ...
      T(559, 176, 'world-space', abs);
      T(559, 188, 'gaussians', abs);
      arr(154, 140, 188, 140);
      circ(171, 124, '2');
      arr(292, 140, 314, 140);
      arr(466, 140, 502, 140);
      circ(484, 124, '3');
      T(242, 176, 'bounces · NEE', abs);
      T(242, 188, 'trace-back', abs);
      T(392, 176, 'fit · relocate', abs);
      T(392, 188, 'add · nudge', abs);
  ...
  window.GRT9T = function (cv) {
    setup(cv, 640, 262, (ctx, hp, H, mono) => {
      const { T, seg, arr, circ, ink, prose, abs } = hp;
      const warm = GRT.figWarm,
        acc = tok('--accw');
      const cam = [38, 150],
        v = [
          [150, 118],
      ...
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.moveTo(cam[0] + 4, cam[1]);
      for (const q of v) ctx.lineTo(q[0], q[1]);
      ctx.stroke();
      ctx.lineWidth = 1;
      for (const q of v) {
        ctx.fillStyle = prose;
      ...
      for (const q of sfx) ctx.lineTo(q[0], q[1]);
      ctx.stroke();
      ctx.setLineDash([]);
      for (const q of sfx) {
        ctx.strokeStyle = prose;
      ...
      for (let i = chain.length - 1; i > 0; i--) {
        ...
        const mx = (a[0] + b[0]) / 2,
          my = (a[1] + b[1]) / 2 + 16,
          an = Math.atan2(b[1] + 16 - my, b[0] - mx);
        ctx.strokeStyle = warm;
        ctx.globalAlpha = 0.7;
        ctx.beginPath();
        ctx.moveTo(a[0], a[1] + 16);
        ...
        ctx.beginPath();
        ctx.moveTo(mx, my);
        ctx.lineTo(mx + 5 * Math.cos(an - 0.42) * -1, my + 5 * Math.sin(an - 0.42) * -1);
        ctx.lineTo(mx + 5 * Math.cos(an + 0.42) * -1, my + 5 * Math.sin(an + 0.42) * -1);
        ctx.fill();
      }
      for (const q of chain) {
        ctx.fillStyle = warm;
        ctx.fillRect(q[0] - 2.4, q[1] + 16 - 2.4, 4.8, 4.8);
        ctx.strokeStyle = warm;
        ctx.beginPath();
        ctx.arc(q[0], q[1] + 16, 4.8, 0, 6.283);
        ctx.stroke();
      }
      circ(196, 158, '3');
      T(
