...
window.GRT2 = (() => {
  ...
  class Cam2 {
    constructor(cv, yaw, pitch, dist) {
      ...
      if (cv) {
        ...
        cv.addEventListener('pointerdown', (e) => {
          if (this.gate && this.gate(e)) return;
          this.dr = true;
          this.auto = 0;
          this.vx = this.vy = 0;
          px = e.clientX;
        ...
        cv.addEventListener('pointermove', (e) => {
          ...
          py = e.clientY;
          this.yaw -= dx * 0.008;
          this.pitch = Math.max(-1.15, Math.min(1.15, this.pitch - dy * 0.006));
          this.vx = 0.5 * this.vx - 0.5 * dx * 0.008;
          this.vy = 0.5 * this.vy - 0.5 * dy * 0.006;
        });
        const up = () => {
          this.dr = false;
          cv.style.cursor = 'grab';
    ...
    step(dt) {
      ...
      if (!this.dr) {
        this.yaw += this.vx;
        this.pitch = Math.max(-1.15, Math.min(1.15, this.pitch + this.vy));
        this.vx *= 0.9;
        this.vy *= 0.9;
      }
    }
