...
window.GRTNEB=(()=>{
const nudge=.9,norm=1/Math.sqrt(1+nudge*nudge);
function spiralC(x,y,z){let n=0,it=2;
for(let i=0;i<8;i++){n+=-Math.abs(Math.sin(y*it)+Math.cos(x*it))/it;
let t=(x+y*nudge)*norm;y=(y-x*nudge)*norm;x=t;
t=(x+z*nudge)*norm;z=(z-x*nudge)*norm;x=t;it*=1.733733}
return n}
function h13(x,y,z){x=(x*.1031)%1;y=(y*.1031)%1;z=(z*.1031)%1;
if(x<0)x+=1;if(y<0)y+=1;if(z<0)z+=1;
const d=x*(z+31.32)+y*(y+31.32)+z*(x+31.32);
x+=d;y+=d;z+=d;const r=((x+y)*z)%1;return r<0?r+1:r}
function vnoise(x,y,z){const i=Math.floor(x),j=Math.floor(y),k=Math.floor(z);
let fx=x-i,fy=y-j,fz=z-k;
fx=fx*fx*(3-2*fx);fy=fy*fy*(3-2*fy);fz=fz*fz*(3-2*fz);
const l=(a,b,t)=>a+(b-a)*t;
const x0=l(l(h13(i,j,k),h13(i+1,j,k),fx),l(h13(i,j+1,k),h13(i+1,j+1,k),fx),fy);
const x1=l(l(h13(i,j,k+1),h13(i+1,j,k+1),fx),l(h13(i,j+1,k+1),h13(i+1,j+1,k+1),fx),fy);
return 1-.92*l(x0,x1,fz)}
const fbm=(x,y,z)=>vnoise(x*.06125,y*.06125,z*.06125)*.5+vnoise(x*.125,y*.125,z*.125)*.25+vnoise(x*.25,y*.25,z*.25)*.125;
const sph=(x,y,z,r)=>Math.hypot(x,y,z)-r;
const xr=(a,b)=>Math.max(Math.min(a,b),-Math.max(a,b));
function cone(x,y,z,h,r1,r2){const qx=Math.hypot(x,z),qy=y;
const k2x=r2-r1,k2y=2*h,dk=k2x*k2x+k2y*k2y;
...
function rot(p,ax,ay,az,ang){const l=Math.hypot(ax,ay,az);ax/=l;ay/=l;az/=l;
const c=Math.cos(ang),s=Math.sin(ang),d=p[0]*ax+p[1]*ay+p[2]*az;
return[p[0]*c+(ay*p[2]-az*p[1])*s+ax*d*(1-c),
...
function butterfly(px,py,pz){const S=4.2;
let p=rot([py*S,pz*S,px*S],-0.1,1,-0.3,Math.PI/3);
const qx=p[0]*1.6,qy=p[1]*1.6,qz=p[2]*1.6;
