...
function alpha(hex,a){const h=hex.replace('#',''),x=h.length===3?h.split('').map(c=>c+c).join(''):h;
...
return{rng,fit,loop,tok,cmap,css,Cam,star,elide,alpha,figWell,figPaper,figWarm};})();
