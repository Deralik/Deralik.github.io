...
(() => {
  ...
  const fl = document.querySelector('.fl[data-k="f1"]');
  if (!fl) return;
  ...
  let sv = {};
  try {
    sv = JSON.parse(localStorage.getItem('cinr-view') || '{}');
  } catch (e) {}
  if (!(sv.zoom > 0)) sv.zoom = 1.37;
  if (typeof sv.lod !== 'number') sv.lod = -0.09;
  const hero = CINR.bunny(cv, {
    mode: 'card',
    onBricks: cvw.feed,
    fresh: cvw.fresh,
    anyFresh: cvw.anyFresh,
    onState: setChip,
    initZoom: sv.zoom,
    initLod: sv.lod,
  });
  const saveV = () => {
    try {
      localStorage.setItem('cinr-view', JSON.stringify(hero.getView()));
    } catch (e) {}
  };
  cv.addEventListener('pointerup', saveV);
  cv.addEventListener('dblclick', saveV);
  ...
  function place() {
    ...
    const pb = fl.parentNode.getBoundingClientRect(),
      fs = fl.getBoundingClientRect();
    fl.style.transition = 'none';
    const rF = target.getBoundingClientRect();
    fl.style.left = fs.left - pb.left + 'px';
    fl.style.top = fs.top - pb.top + 'px';
    fl.style.width = fs.width + 'px';
    fl.style.height = fs.height + 'px';
    void fl.offsetWidth;
    fl.style.transition = '';
    fl.style.left = fin.L;
    fl.style.top = fin.T;
    fl.style.width = fin.W;
    fl.style.height = fin.H;
    /* glide the canvas in viewport space on the same curve — it lands
     exactly when and where the slot does */
    cv.style.position = 'fixed';
    cv.style.inset = 'auto';
    cv.style.zIndex = 4;
    cv.style.left = r0.left + 'px';
    cv.style.top = r0.top + 'px';
    cv.style.width = r0.width + 'px';
    cv.style.height = r0.height + 'px';
    cv.style.transition = 'none';
    void cv.offsetWidth;
    cv.style.transition = 'left var(--t),top var(--t),width var(--t),height var(--t)';
    cv.style.left = rF.left + 'px';
    cv.style.top = rF.top + 'px';
    cv.style.width = rF.width + 'px';
    cv.style.height = rF.height + 'px';
    clearTimeout(place._t);
    place._t = setTimeout(() => {
      cv.style.cssText = '';
    }, 380);
  }
  const animBtn = $('#cinr-anim'),
    resetBtn = $('#cinr-reset');
  if (animBtn)
    animBtn.addEventListener('click', () => {
      animBtn.textContent = 'Animation · ' + (hero.toggle() ? 'on' : 'off');
    });
  if (resetBtn)
  ...
  const ordBtn = $('#cinr-order');
  let ord = 'rank';
  if (ordBtn)
    ordBtn.addEventListener('click', () => {
      ord = ord === 'rank' ? 'fifo' : 'rank';
      cvw.setOrder(ord);
      ordBtn.textContent = 'Fill order · ' + (ord === 'rank' ? 'ranked' : 'unranked sweep');
    });
  if (capIn) capIn.addEventListener('input', () => cvw.setCap(capIn.value / 100));
  if (lodIn && hero) {
    lodIn.value = Math.round(sv.lod * 100);
    lodIn.addEventListener('input', () => {
      hero.setLod(lodIn.value / 100);
      saveV();
    });
  }
  ...
  const lg = (el, names) => {
    if (el)
      el.innerHTML = names
        .map(
          (n) =>
            '<span><i class="csw" style="background:' +
            CINR.DS(CINR.DSN[n]) +
            '"></i>' +
            n +
            '</span>',
  ...
  if (tb) {
    const gmax =
      Math.max.apply(
        null,
        CINR.T.map((r) => r[1] / r[2]).concat(CINR.T.map((r) => r[3] / r[4])),
      ) * 1.28;
    const cell = (r, i, a, b) => {
      const g = r[a] / r[b];
      return (
        ...
        r[a].toFixed(1) +
        '</em>' +
        '<span class="cbar"><b style="width:' +
        ((g / gmax) * 100).toFixed(1) +
        '%;background:' +
        CINR.DS(i) +
        '"></b><i>' +
        g.toFixed(2) +
    ...
    const bySize = CINR.T.map((r, i) => [r, i]).sort(
      (a, b) =>
        a[0][5][0] * a[0][5][1] * a[0][5][2] * (a[0][0] === 'DNS' ? 8 : 4) -
        b[0][5][0] * b[0][5][1] * b[0][5][2] * (b[0][0] === 'DNS' ? 8 : 4),
    );
    const rows = bySize
      .map(
        ([r, i]) =>
      ...
      .join('');
    tb.innerHTML =
      '<div class="ctr cth"><span>dataset</span><span class="cdim">size</span><span>ray-march · INR → cached FPS · gain</span><span>path-trace · INR → cached FPS · gain</span></div>' +
      rows;
    const mm = (sel) => {
      const a = CINR.T.map(sel);
      return (a.reduce((s, v) => s + v, 0) / a.length).toFixed(2);
    };
    const nts = tb.nextElementSibling;
    if (nts && nts.classList.contains('cnotes'))
      nts.insertAdjacentHTML(
        'afterbegin',
        '<span>row means, derived: ' +
          mm((r) => r[1] / r[2]) +
          '× ray-march · ' +
          mm((r) => r[3] / r[4]) +
          '× path-trace — the basis of the ~5× and ~2× above</span>',
      );
  }
  ...
  new MutationObserver(setFocus).observe(fl, {
    attributes: true,
    attributeFilter: ['data-face', 'class'],
  });
  new MutationObserver(setFocus).observe(document.body, {
    attributes: true,
